...
def agent_app(monkeypatch: pytest.MonkeyPatch) -> AgentEngineApp:
    ...
    # Mock Runner.run to avoid hitting remote Vertex AI/Gemini endpoints during template testing
    import google.adk.runners
    from google.adk import Event
    from google.genai import types

    def mock_run(self, *args, **kwargs):
        yield Event(content=types.Content(parts=[types.Part.from_text(text="Mocked integration response")]))

    monkeypatch.setattr(google.adk.runners.Runner, "run", mock_run)
